...
bool ListAllTransactionsisEmpty(const StackAllTransactions& L) {
	return L.size == 0;
}

int ListAllTransactionsSize(const StackAllTransactions& L) {
	return L.size;
}

bool ListAllTransactionsisFull(const StackAllTransactions& L) {
...
StackAllTransactions createListAllTransactions() {
	return StackAllTransactions{ nullptr, 0 };
}


void destroyListAllTransactions(StackAllTransactions* L) {
	if (!L) return;
	NodeAllTransactions* current = L->head;
	while (current) {
...
void displayListAllTransactions(const StackAllTransactions& L) {
	if (ListAllTransactionsisEmpty(L)) {
		cout << "List is empty\n";
		return;
	}
...
StackAllTransactions CopyStackAllTransactions(const StackAllTransactions& L) {
	StackAllTransactions newStackAllTransactions = createListAllTransactions();
	NodeAllTransactions* current = L.head;
	...
	while (current) {
		...
		if (!n) {
			...
			destroyListAllTransactions(&newStackAllTransactions);
...
bool CompareListAllTransactions(const StackAllTransactions& L1, const StackAllTransactions& L2) {
